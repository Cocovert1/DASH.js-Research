...
app.controller('DashController', ['$scope', '$window', 'sources', 'contributors', 'dashifTestVectors', function ($scope, $window, sources, contributors, dashifTestVectors) {
    ...
    // CHANGE: changed interval of print to 8 seconds
    ...
    $scope.doStop = function () {
        ...
        $scope.controlbar.reset();
        $scope.conformanceViolations = [];
        // CHANGE: added to csv and download the file.
        console.log(csvDataArray);
        csvDataArray.forEach(function (rowArray) {
    ...
    function calculateHTTPMetrics(type, requests) {
        ...
        var requestWindow = requests.slice(-20).filter(function (req) {
            // CHANGE: changed slice to -1 so that the average download is every second
    ...
    // CHANGE: added array to hold values/ created csv file
    let csvDataArray = [];
    ...
    function updateMetrics(type) {
        ...
        if (dashMetrics && $scope.currentStreamInfo) {
            ...
            // CHANGE: %2 to %1 so that it updates every time
            if ($scope.chartCount % 1 === 0) {
                ...
                if (httpMetrics) {
                    ...
                    //  CHANGE: got the metrics
                    var dwl = httpMetrics.download[type].average.toFixed(2);
                    // seconds * mbps to bits / to bytes
                    var seg = (httpMetrics.duration * ($scope.videoBitrate / 1000) * 1000000) / 8;

                    // Generate CSV data and add to the array
                    var data = Object.values({
                        time,
                        bitrate: ($scope.videoBitrate / 1000),
                        buffer: $scope.videoBufferLength,
                        throughput: ($scope.videoMtp),
                        download: httpMetrics.download[type].average.toFixed(2),
                        segmentSize: (seg * 8),
                    });

                    console.log($scope.videoMtp);

                    csvDataArray.push(data);

                    // console.log("Bitrate: ", ($scope.videoBitrate / 1000), "Mbps");
                    // console.log("Buffer: ", $scope.videoBufferLength, "seconds");
                    // console.log("throughput", ($scope.videoMtp), "Mbps");
                    // console.log("Download: ", httpMetrics.download[type].average.toFixed(2), "seconds");
                    // console.log("Segment Size: ", seg, "Kbit to byte");
